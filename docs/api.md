...
工单类型用于对工单进行分类，方便后续统计和筛选。仅管理员可操作。
...
| ----------- | ------ | --------------------------------- |
| id          | int64  | 工单类型 ID                       |
| code        | string | 类型编码，唯一标识                |
| name        | string | 类型名称                          |
| description | string | 类型描述（可能为 null）           |
...
| 字段        | 类型   | 必填 | 说明                        |
| ----------- | ------ | ---- | --------------------------- |
| code        | string | 是   | 类型编码，全局唯一          |
| name        | string | 是   | 类型名称                    |
| description | string | 否   | 类型描述                    |
| color       | string | 否   | 颜色标识，HEX 格式，如 `#EF4444` |
| status      | string | 否   | 状态，默认 `active`         |

**成功响应** (201): 返回创建的工单类型对象。
...
手动创建工单（不通过告警源），用于内部申报或测试场景。

**认证方式**: JWT Bearer Token（admin/operator 均可）
...
| 字段           | 类型   | 必填 | 说明                              |
| -------------- | ------ | ---- | --------------------------------- |
| ticket_type_id | int64  | 是   | 工单类型 ID                       |
| title          | string | 是   | 工单标题                          |
| description    | string | 否   | 工单描述                          |
| severity       | string | 是   | 严重程度：`critical` / `warning` / `info` |
| client_id      | int64  | 否   | 关联客户 ID（可选）               |

**成功响应** (201): 返回创建的工单对象（与 `GET /tickets/:id` 响应中的 `ticket` 字段结构一致）。

**错误响应**:

| 状态码 | 说明                              |
| ------ | --------------------------------- |
| 400    | 缺少必填字段、工单类型不存在或非 active、客户不存在或非 active |
| 401    | 未认证                            |
| 500    | 创建失败                          |
...
| -------- | ------ | ---- | -------------------------- |
| username | string | 是   | 用户名                     |
| password | string | 否   | 新密码，不填则保留原密码   |
| role     | string | 是   | 角色：`admin` / `operator` |
