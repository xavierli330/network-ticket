...
工单类型用于对工单进行分类，方便后续按类型筛选和统计。

### 3.1 创建工单类型

进入「工单类型管理」页面，点击「新建工单类型」，填写以下信息：

| 字段 | 必填 | 说明 |
|------|:----:|------|
| 编码 (code) | ✅ | 类型唯一编码，如 `incident`、`request`、`maintenance` |
| 名称 (name) | ✅ | 类型显示名称，如 "故障工单" |
| 描述 | ❌ | 类型的简要说明 |
| 颜色 | ❌ | 类型标识颜色，HEX 格式，如 `#EF4444` |
| 状态 | ❌ | `active`（默认）/ `inactive` |

**常用工单类型示例**：

| 编码 | 名称 | 颜色 | 说明 |
|------|------|------|------|
| `incident` | 故障工单 | `#EF4444`（红） | 系统故障、服务中断 |
| `request` | 服务请求 | `#3B82F6`（蓝） | 资源申请、配置变更 |
| `maintenance` | 维护工单 | `#10B981`（绿） | 计划内维护、巡检 |
...
**筛选条件**：
- 按状态筛选：全部状态 / 待处理 / 处理中 / 已完成 / 失败 / 已取消 / 已拒绝
- 按严重级别筛选：全部级别 / 严重 / 警告 / 信息
- 按工单类型筛选：全部类型 / 各已创建类型
- 关键词搜索：支持按工单标题模糊搜索
...
**基本信息**：工单编号、标题、描述、严重程度、工单类型、告警源类型、创建时间、更新时间。

**当前状态**：
- 状态图标和名称（如 🟡 等待推送）
- 推送状态（⏳ 尚未推送 / ✅ 已推送）
- 待处理人（系统自动推送 / 指定人员）
- 状态说明（如"工单已创建，未关联客户，无法自动推送"）

**完整流程状态**（可展开）：
- 7 节点工作流追踪：alert_received → parsed → pushed → awaiting_auth → authorized → executing → completed
- 每个节点状态：pending / active / done / failed / skipped / timeout
...
除了通过告警源自动创建工单外，管理员和操作员还可以手动创建工单，适用于内部申报、测试或无需监控系统的场景。

**操作路径**：工单管理 → 点击「新建工单」按钮
...
| 关联客户 | ❌ | 可选，选择后工单会尝试推送给该客户 |

手动创建的工单与自动创建的工单在后续处理流程上完全一致，支持重试、取消、状态变更等操作。
...
用户管理用于创建和维护平台登录账号。仅管理员可操作。

### 7.1 创建用户

进入「用户管理」页面，点击「新增用户」，填写以下信息：
