...
2. **添加工单类型** — 「工单类型」→ 新建类型（如 `network_fault` 网络故障、`server_alert` 服务器告警）
3. **添加客户** — 「客户管理」→ 填写客户名称、推送地址、API Key、HMAC Secret
4. **添加告警源** — 「告警源管理」→ 选择类型（Zabbix / Prometheus / 通用），配置 Webhook 地址到监控系统
...
- **工单类型管理** — 自定义工单分类（编码、名称、颜色），支持按类型筛选
- **搜索与筛选** — 关键词搜索、状态/级别/类型多条件组合筛选
- **工单详情页** — 查看完整工单信息、当前状态、推送状态
...
| [使用指南](docs/usage.md) | 管理员 / 操作员 | 告警源配置、客户对接、工单操作、工单类型管理 |
...
| [已知问题](docs/known-issues.md) | 所有用户 | 当前版本已知 Bug 和临时解决方案 |
